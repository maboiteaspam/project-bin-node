...
tasksGitHelper.addOrigin = function(name, remote) {
  return tasksUtils
    .spawnProcess(name, 'git remote add origin ' + remote, {
      failOnError:true
    });
}
tasksGitHelper.setOrigin = function(name, remote) {
  return tasksUtils
    .spawnProcess(name, 'git remote set-url origin ' + remote, {
      failOnError:true
    });
}
tasksGitHelper.setUpstream = function(name, branch) {
  branch = branch || 'master'
  return tasksUtils
    .spawnProcess(name, 'git push --set-upstream origin ' + branch, {
      failOnError:true
    });
}
